...
export function getSpendableSkillPoints(level, intellect) {
  const L = clampLevel(level);
  const intScore = toInt(intellect ?? 0, { min: 0, max: 99 });
  return 2 * (L + intScore);
}

export function getSkillPointCostForRank(rank, { grantedRank = 0 } = {}) {
  const finalRank = toInt(rank ?? 0, { min: 0, max: 6 });
  const freeRanks = toInt(grantedRank ?? 0, { min: 0, max: 6 });
  return Math.max(0, finalRank - freeRanks);
}

...
const elements = {
  ...
  skillPointsRemainingPill: document.getElementById("skillPointsRemainingPill"),
  skillRankCap: document.getElementById("skillRankCap"),
  classUtilitySkillsCard: document.getElementById("classUtilitySkillsCard"),
  classUtilitySkillsMeta: document.getElementById("classUtilitySkillsMeta"),
...
function issues(reconciliation) {
  return {
    ...
    confirmations: reconciliation.impacts.filter((item) => item.category === "confirmation-required" && item.code.startsWith("skill-")),
  };
}

function renderNav() {
  renderBuilderNavMounts({ currentStepId: CURRENT_STEP_ID, characterDoc: currentDoc, ctx: { charId: ctx.charId, requestedUid: ctx.requestedUid }, onBeforeNavigate: async () => saveBuilder({ intent: "navigate" }) });
}
...
async function saveBuilder({ openSheetAfter = false, intent = "save" } = {}) {
  ...
  if (!visited.ok) {
    showError(elements.error, visited.errors.join(" ") || "The skill state could not be reconciled.");
  ...
  if (currentIssues.warnings.length && !await confirmSaveWarnings({ title: "Save with incomplete skills?", warnings: currentIssues.warnings, okText: intent === "navigate" ? "Save and Continue" : "Save", cancelText: "Cancel" })) {
    setStatus(elements.status, "Not saved.");
    return false;
  }
  ...
  if (!saved.ok) {
    showError(elements.error, saved.error?.code === "character-revision-conflict" ? "This character changed in another tab. Reload before saving again." : "Could not save skills.");
...
async function main() {
  try {
    ...
    page = new CharacterSessionPage({
      ...
      confirmImpacts: ({ messages }) => confirmSaveWarnings({ title: "Apply this skill change?", warnings: messages, okText: "Apply Change", cancelText: "Cancel" }),
      onCommandRejected: ({ errors }) => { showError(elements.error, errors.join(" ") || "That skill change is not valid."); setStatus(elements.status, "Change rejected."); },
      onStateChange: (state) => { currentDoc = state.working; clearError(elements.error); setStatus(elements.status, "Unsaved changes."); },
    });
    widget = new SkillsWidget(page, { gameData, elements, onRejected: () => widget.render() });
    const preview = issues(reconcileCharacterGraph({ character: currentDoc, previousCharacter: currentDoc, gameData }));
    if (preview.errors.length) {
      showError(elements.error, `Stored skills need review. ${preview.errors.join(" ")}`);
      setStatus(elements.status, "Review needed.");
    } else if (preview.confirmations.length) {
      showError(elements.error, "Stored skill ranks exceed current caps or the point budget. Save or edit a skill to review the exact adjustments.");
      setStatus(elements.status, "Review needed.");

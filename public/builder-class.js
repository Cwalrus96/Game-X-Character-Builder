...
function kebabToSheetClassValue(classKey) {
  return String(classKey || "").replace(/-/g, "");
}
...
async function saveClassStep({ openSheetAfter = false } = {}) {
  ...
  try {
    // Ensure sheet maps exist (for local cache usage)
    currentDoc.sheet = currentDoc.sheet || {};
    currentDoc.sheet.fields = currentDoc.sheet.fields || {};
    currentDoc.sheet.repeatables = currentDoc.sheet.repeatables || {};

    const clsValue = kebabToSheetClassValue(selectedClassKey);
    ...
    const existingAbilities = currentDoc?.sheet?.repeatables?.abilities || [];
    ...
    const patch = {
      ...
      "sheet.fields.level": clampLevel(selectedLevel),
      "sheet.fields.classSelect": clsValue,
      "sheet.fields.primaryAttribute": safeStr(selectedPrimary, 32),
      "sheet.repeatables.abilities": mergedAbilities,
    };
    ...
    currentDoc.sheet.fields = {
      ...(currentDoc.sheet.fields || {}),
      level: clampLevel(selectedLevel),
      classSelect: clsValue,
      primaryAttribute: selectedPrimary,
    };
    currentDoc.sheet.repeatables = {
      ...(currentDoc.sheet.repeatables || {}),
...
async function main() {
  try {
    ...
    // Backfill builder.classKey from sheet.fields.classSelect if needed.
    if (!selectedClassKey) {
      const sheetVal = String(currentDoc?.sheet?.fields?.classSelect || "");
      if (sheetVal) {
        const match = classes.find((c) => kebabToSheetClassValue(c.classKey) === sheetVal);
        if (match) selectedClassKey = match.classKey;
      }
    }

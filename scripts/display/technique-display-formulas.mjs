...
export const TECHNIQUE_COMPATIBILITY_HEADERS = Object.freeze([
  ...
  'prerequisiteText', 'skillKeys', 'tagKeys', 'associatedSkill',
]);
...
const actionText = compact(`IF(OR(get("actionType")="",get("actions")=""),"Actions: Unassigned",
  IF(get("actions")="0","Free",get("actions"))&" "&IF(get("actionType")="ActionOrReaction","Action or Reaction",get("actionType")))`);
...
export function prerequisiteTextFormula(expression, {
  techniques = "'_Techniques'!A1:AJ1000",
  ...
  return compact(`LET(prerequisitetext,${expression},IF(prerequisitetext="","",TEXTJOIN("; ",TRUE,
    MAP(TRANSPOSE(SPLIT(prerequisitetext,CHAR(10),FALSE,TRUE)),LAMBDA(rawline,LET(
      ...
      allowed,SWITCH(kind,"weapon","key|tag|tagAll|tagAny|tagNot|minReach|wielded",
        ...
        "archetype","key|archetypeKey|numFeats","text","text",""),
      ...
      MAP(${keys},LAMBDA(key,LET(matches,SUM(ARRAYFORMULA(N(sourcekeys=key))),
        IF(matches<>1,"[Unresolved technique key: "&key&"]",LET(
          ...
          TEXTJOIN(CHAR(10),TRUE,
            ...
            usage,cost,IF(get("trigger")="","","Reaction Trigger: "&get("trigger")),
...
export function techniqueNamedFunctionPatches() {
  ...
  return {
    TECHNIQUE_GET_FIELD: compact(`=LET(source,'_Techniques'!A1:AJ1000,head,INDEX(source,1,0),
...
export function techniqueNativeFixtures() {
  ...
  return [
    { name: 'Technique optional headers produce full blank columns', formula: `=AND(ROWS(${compat})=3,COLUMNS(${compat})=36,INDEX(${compat},2,24)="",INDEX(${compat},3,36)="")`, expected: true },

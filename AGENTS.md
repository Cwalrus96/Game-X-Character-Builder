...
- Export all records. Authoring uses `Metadata.readinessPolicy=required-cells-v1`: no manually maintained readiness/status columns. Derive completeness only from declared required cells, never optional descriptions, notes, Energy, or pumping. Blank Energy means zero; pumping exists only when `pumpingByRank` is explicitly authored. Preserve currently playable content when changing validation. Technique `selection` and enhancement `selectionMode` describe acquisition only. Runtime status remains derived compatibility data; earlier v5/v4 snapshots retain their own adapter policy.
...
- Authoring schema v5/syntax v3 `Metadata`, `Schema`, and `Enums` are contract inputs, not editorial tabs. Versioned adapters accept v5/v3 and preserve the v4/v2 compatibility path. The reviewed production release uses runtime schema 3. Each subsequent publication still requires a complete validated candidate and explicit approval; source/display acceptance alone does not authorize publishing.
...
- Add valid and invalid fixtures for each schema, grant, prerequisite, migration, or reconciliation rule. Unit tests use small controlled fixtures and must not read published game content or depend on particular spreadsheet rows/counts. Checks that exercise the installed catalogue belong in the separate `npm run test:data` release integration suite.

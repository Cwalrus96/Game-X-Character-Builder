...
Continuation checkpoint: the [release review](game-data-release-review-2026-09-21.md) compares a fresh read-only capture of all 13 current-path characters. Eight reconcile cleanly, one has an existing equipment error, two require content-change review, and two have retired references requiring compatibility handling. False skill-budget reductions are repaired. Weapon-use eligibility and retired Metamorph review policy await the user's answers; no source or production mutation occurred. The full local candidate website passes the new-character save/reload, cancellation/acceptance and stale-save scenarios. This step remains active.

September 22 supersedes those pending decisions: static weapon eligibility ignores wielding/hands, Celestial Knight's known obsolete child is absorbed by its selected both-skills parent, and old Metamorph requires a player rebuild with the original preserved. Fresh run `20260922T184507461Z-43412` has zero errors/254 warnings. Replay now has eight clean records, one existing capacity error, three ordinary rule-change reviews and one explicit rebuild. Final verification passes 431 unit tests, 20 emulator tests, 14 asset checks and local Celestial save/reload/cancellation plus Metamorph preservation. A matching 128-file Hosting package is prepared. Remaining acceptance is exact candidate publishing/Hosting approval and coordinated promotion, not a wait for personal manual testing.
...
September 22 feat-widget refinement: both compact dropdowns and expanded description cards now always exclude ineligible feats. Each widget has an accessible Expand/Collapse control; expansion survives ordinary page rerenders without changing saved character data. Expanded cards show full multiline text, prerequisites and selection controls, while selected follow-up choices remain source-owned. Four additional regression tests and browser checks cover filtering, both views, keyboard selection, cancellation/focus, retained expansion and save/reload. Full verification passes 456 units, 20 emulator tests and 14 asset checks; this remains a local website change with a separate Hosting release boundary.

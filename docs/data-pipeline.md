...
The September 22 source correction declares `Metadata.readinessPolicy=required-cells-v1` and removes Classes, Origins, and Techniques status columns. `scripts/authoring/required-cell-migration.mjs` prepares the pure source-row migration; the caller owns inspected native coordinates, prewrite comparison, and exact readback. The importer selects the corresponding 112-field contract while retaining historical v5/v4 paths. Completeness uses only declared required cells. Blank Energy defaults to zero; blank pumping never creates effects or blocks availability. Status-free source values remain preserved alongside generated readiness evidence. Do not repair missing optional costs by inserting zeroes into source cells.

Before publishing this policy, compare every existing selectable/executable record against both the pre-migration source and published catalogue. A complete fresh export must validate, runtime-load, and produce the ten-file staged diff. Source migration, a local candidate, and display acceptance do not authorize production publication or Hosting deployment.
...
The original Trait migration was an authoring/display extension, with local plans and recovery evidence ignored under `.staging/traits-migration-2026-09-20/`. The subsequent v5 importer adapts and validates Traits, provider `traitKeys`, and unresolved rank/access data. Syntax-v3 `trait` grants now carry explicit fixed-rank or skill context, recipient, choice identity, and permanent/toggle activation; incomplete and unsupported providers remain deferred. Character Trait state and execution belong to the separately versioned builder contract. The [exact Origin-provider source proposal](trait-source-proposals-2026-09-21.md) remains subject to source-edit approval. Neither implementation publishes JSON or authorizes production deployment. Linked-table publication into the handbook does not satisfy `stage:data` acceptance.
...
Finishing all game content is not a prerequisite for import. Intentional drafts and unknown mechanics remain represented with precise source-located readiness/runtime-support warnings; invalid nonblank expressions, scalars, references, or structural loss remain errors. Deferred mechanics are not selectable merely because their source parses. Conditional evolution, generic Keystone choice execution, recipient-owned skill state, and unfinished Familiar/Mech/provider rules remain separate execution work. The September 22 static weapon policy preserves authored wielding/separate-hand qualifiers for display without deferring execution or tracking gameplay hands. Eligibility checks matching owned equipment; candidate-weapon enhancement scope is unchanged. Source and runtime artifact versions do not set the saved-character schema; its versioned state and migration contract is documented separately.

The staged semantic diff must be reviewed before release, especially deleted stable identities and retired weapon profiles. The narrow Integrated Weapon alias preserves its known identity change; unrelated removed keys are not guessed from names. The publisher now supports both source-v4/runtime-v2 and source-v5/runtime-v3 approvals with their exact nine- and ten-file sets. The checked-in production approval and baseline still describe the existing schema-v2 release. `WPB-V5-RELEASE-REVIEW` must finish candidate and compatibility review before separate approval of any new bytes.
...
It also verifies that the currently installed production bytes still match their existing baseline and reruns runtime acceptance against the candidate. A schema-v2 approval must contain exactly the nine established artifact names; schema v3 must contain those nine plus `traits.json`. The staged artifact directory and export report must have the same exact file set, without extras, duplicates, directories, or substituted files. Validation/export versions, normalized-model hash, provenance, and combined-artifact source revision must agree with the approval. An explicitly recorded null Drive version is allowed for source v5; a missing or blank version is not equivalent to that explicit unknown, and the older v4 approval still requires a nonempty version.

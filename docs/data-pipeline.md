...
Status: living operational design. Authoring schema v5/syntax v3 is ahead of the implemented schema-v4/syntax-v2 acquisition and runtime adapter. Existing read/validate/stage/diff, reviewed publishing, and rollback remain unchanged; the source/display cleanup does not publish runtime data.
...
The source is [game-x-class-data](https://docs.google.com/spreadsheets/d/1TEdxuufglP8lFRNk8QD4N_351-0ihAUFLG2743ESjoI/edit). Its non-secret locator and implemented acquisition expectations live in `contracts/game-data-source.json`. That file still expects schema v4 and the old required tabs; the accepted authoring schema is v5. This is a deliberate runtime integration boundary, not a reason to bypass validation or recreate retired source tabs.
...
The implemented command fetches the canonical Sheet, requires schema v4, constructs deterministic runtime artifacts in memory, verifies them through current runtime loader APIs, and installs a complete unique run under the path below. The current schema-v5 authoring source is not yet compatible; no successful source/display check removes that block.
...
The September 21 cleanup deliberately adds further explicit boundaries: authoring schema/syntax versions advance to 5/3, ClassSkills and WeaponProfiles are retired from source, and Technique access/readiness/pumping, formal Trait prerequisites, and recipient tag grants have new authored representations. Runtime acquisition and adaptation must be updated together later, including saved-state compatibility and graph/Rules semantics where needed. Do not change version checks alone, recreate duplicate authored columns, or hand-edit production JSON to make a staging command pass.

The same future integration must preserve the follow-up's retired grant summaries, `basicAttack` references/overrides, `ActionOrFreeReaction`, typed-clause prerequisite alternatives, distinct-known-option counts, source-owned feature invocations, and explicit skill recipients. It must validate reference targets and invocation cycles while retaining conditional rules whose execution is still held. Do not flatten a feature reference into an unconditional grant, inherit its old parent/level, or assign an Artifact's skills to the character.

Finishing all game content is not a prerequisite for that migration. The importer must represent intentional drafts, unknown mechanics, and supported-but-unimplemented rules with stable identities and precise source-located diagnostics, preserving their readiness restrictions. Distinguish these findings from malformed expressions, invalid scalars, broken references, or structural loss. The staged report must make those categories reviewable without coercing blanks, dropping records, or forcing the author to design a missing rule merely to test acquisition/adaptation. Runtime execution and publishing still require their respective validation and approval boundaries; the current v4/v2 command has not gained these capabilities from documentation or display changes.

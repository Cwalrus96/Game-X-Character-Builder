...
| `L1:L16` | Weapon techniques, including 15 incomplete techniques and the existing Deflect Projectile technique |
...
The canonical `Techniques!A87:AI98` holds the 12 complete techniques migrated on September 18. The September 19 authoring migration moved the 15 new unfinished weapon techniques into `Techniques!A99:AI113`, retaining their stable keys, original prose, questions, and provenance. They use `selectionMode=draft`; unresolved ranks, skill access, and numeric costs remain blank, with `energyCostKind=unassigned`. Deflect Projectile retains its existing canonical row, with the original reference wording preserved in notes. The separate `TechniqueDrafts` tab was removed. `Techniques_Display!A1` now enumerates every canonical technique key instead of grouping only records with assigned skills; unknown rank/access render as **Rank TBD** and **Access: Unassigned**, without invented free/zero-energy mechanics. Column L resolves its original 16 keys from this same catalogue. Rank 0 excerpts follow current canonical damage and pumping rules.

For feats, `_Feats!A1` normalizes the imported columns by header name; `Feats_Display!A2` compacts class-feat rows before formatting. `Feats_Display` became empty because its consumers interpreted the reordered source columns by position. Maintain the header lookup rather than restoring positional assumptions. Native handbook links cover 22 class feats and 49 archetype memberships. The Weapon Master ranges are `Feats_Formatted!E2:E8` and `Archetypes_Formatted!E1:E9`. Refresh and expand native links when their source ranges grow.

All archetype membership is authored in canonical `Feats`: column N `archetypeKey` is the Archetype Member dropdown, and column O `archetypeName` supplies the display label once on the entry feat. Each top-level member reuses the stable key; nested OPTION rows inherit membership through `parentKey`. `category` supplies the subclass's class or `multiclass`. Prior-feat requirements are written once in `prerequisites` as `archetype | <archetypeKey> | numFeats=N`; absence of a same-group count condition means zero. Existing stable keys are preserved even where labels changed (for example `sparkling-idol` / Shining Protector). The separate canonical `ArchetypeFeats` tab was removed. The hidden display `_ArchetypeFeats` is now a generated six-column compatibility view imported directly from canonical `Feats`, not an editable second source. It must not depend on `_Feats`, because `_Feats` already uses it to render prerequisite labels.
...
Inspection during the September 18–19 handbook migrations found live-source publishing blockers: `Feats` headers and its new archetype fields do not match the current adapter contract, archetype DSL is unsupported by the runtime registry, and incomplete Techniques may have blank ranks/skills. The former `ArchetypeFeats` source tab has been consolidated into Feats. The user explicitly deferred exporter compatibility; no live `stage:data` run or runtime publish was performed for these migrations. Resolve the source/runtime contract before preparing a new release candidate.

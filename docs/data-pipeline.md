...
Finishing all game content is not a prerequisite for import. Intentional drafts and unknown mechanics remain represented with precise source-located readiness/runtime-support warnings; invalid nonblank expressions, scalars, references, or structural loss remain errors. Deferred mechanics are not selectable merely because their source parses. Conditional evolution, generic Keystone choice execution, recipient-owned skill state, and unfinished Familiar/Mech/provider rules remain separate execution work. Syntax-v3 wielded/separate-hand prerequisites also require actual equipment evidence; owning a weapon alone does not prove these conditions. Source and runtime artifact versions do not set the saved-character schema; its versioned state and migration contract is documented separately.
